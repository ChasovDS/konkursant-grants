...
import RoleModal from './ComponentsAdminPage/RoleModal'; // Импортируем модальное окно
...
const UserTable = () => {
    ...
    const fetchUsers = async (page, limit) => {
        ...
        try {
            ...
            setTotalUsers(parseInt(response.headers['x-total-count'], 10) || 0);
        } catch (error) {
    ...
    return (
        <Box>
            ...
            <FormControl variant="outlined" style={{ margin: '10px', minWidth: 250 }}>
                ...
                >
                    ...
                    {Object.keys(roleTranslations).map(role => (
                        <MenuItem key={role} value={role}>{roleTranslations[role]}</MenuItem>
                    ))}
                </Select>
            </FormControl>
                                            ...
                                            <TableRow key={user.user_id}>
                                                ...
                                                <TableCell>
                                                    <Tooltip title="Просмотр профиля">
                                                        <IconButton color="primary" onClick={() => handleViewProfile(user.user_id)}>
                                                            <VisibilityIcon />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Назначить роль">
                                                        <IconButton color="secondary" onClick={() => handleOpenRoleModal(user)}>
                                                            <SecurityIcon />
                                                        </IconButton>
                                                    </Tooltip>
                                                </TableCell>
            ...
            <RoleModal 
                ...
                user={selectedUser} 
                onRoleUpdated={() => fetchUsers(page, rowsPerPage)} 
            />
        </Box>
    );
};
